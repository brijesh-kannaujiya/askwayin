...
@section('content')
    ...
    <div class="card">
        ...
        <h5 class=" mb-0 text-gray-800 pl-3">{{ __('SubscriberUser List') }}</h5>
        <ol class="breadcrumb py-0 m-0">
            <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{ __('Dashboard') }}</a></li>

            <li class="breadcrumb-item"><a href="{{ route('admin.SubscriberUser.index') }}">{{ __('SubscriberUser List') }}</a></li>
        </ol>
        </div>
    </div>
...
<script type="text/javascript">
    'use strict';
        ...
        var table = $('#geniustable').DataTable({
               ...
               serverSide: true,
               searching: true,
               ajax: '{{ route('admin-SubscriberUser-datatables') }}',
               columns: [
                        { data: 'name', name: 'name' },
