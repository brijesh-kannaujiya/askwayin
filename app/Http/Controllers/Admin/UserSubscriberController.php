...
class UserSubscriberController extends Controller
{
    ...
    {
        $datas = User::orderBy('id', 'desc')->where('plan_id', '!=', NULL);
        //dd($datas);
        return Datatables::of($datas)
            ->addColumn('action', function (User $data) {
                return '<div class="btn-group mb-1">
                                    ...
                                    <a href="' . route('admin-SubscriberUser-show', $data->id) . '"  class="dropdown-item">' . __("Details") . '</a>
                                    <a href="' . route('admin-SubscriberUser-edit', $data->id) . '" class="dropdown-item" >' . __("Edit") . '</a>
                                    <a href="javascript:;" class="dropdown-item send" data-email="' . $data->email . '" data-toggle="modal" data-target="#vendorform">' . __("Send") . '</a>
                                    <a href="javascript:;" data-toggle="modal" data-target="#deleteModal" class="dropdown-item" data-href="' .  route('admin-SubscriberUser-delete', $data->id) . '">' . __("Delete") . '</a>
                                    </div>
                                </div>';
            })
            ...
            ->addColumn('status', function (User $data) {
                ...
                return '<div class="btn-group mb-1">
                                    ...
                                    <div class="dropdown-menu" x-placement="bottom-start">
                                        <a href="javascript:;" data-toggle="modal" data-target="#statusModal" class="dropdown-item" data-href="' . route('admin-SubscriberUser-ban', ['id1' => $data->id, 'id2' => 0]) . '">' . __("Unblock") . '</a>
                                        <a href="javascript:;" data-toggle="modal" data-target="#statusModal" class="dropdown-item" data-href="' . route('admin-SubscriberUser-ban', ['id1' => $data->id, 'id2' => 1]) . '">' . __("Block") . '</a>
                                    </div>
                                    </div>';
            })
    ...
    {
        return view('admin.SubscriberUser.index');
    }
    ...
    public function show($id)
    {
        ...
        $data['data'] = $data;
        return view('admin.SubscriberUser.show', $data);
    }
    ...
    {
        $data = User::findOrFail($id);
        return view('admin.SubscriberUser.edit', compact('data'));
    }
